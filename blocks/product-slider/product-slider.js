...
export default function decorate(block) {
  ...
  // Build each card's markup inline — slider-product-card is not a top-level
  // EDS block so its own JS/CSS would never be auto-loaded by decorateBlocks().
  [...block.querySelectorAll('.slider-product-card')].forEach(buildCard);

  // Collect cards into a horizontally scrollable track.
  const track = document.createElement('div');
  track.className = 'ps-track';
  [...block.querySelectorAll('.slider-product-card')].forEach((card) => track.append(card));
